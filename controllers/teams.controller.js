...
const createTeamSchema = Joi.object({
  ...
  if (value.date && value.day) {
    const providedDay = new Date(value.date).toLocaleDateString("en-US", {
      weekday: "long",
    });
...
const createTeam = async (req, res, next) => {
  try {
    ...
    payload.endDate = getNextDueDate(
      payload.date,
      payload.day,
      payload.totalWeek
    );
...
const getAllTeams = async (req, res, next) => {
  try {
    ...
    const conditions = [{ isActive: true }, { day }];
    if (search) {
      conditions.push({
        $expr: {
          $regexMatch: {
...
const editTeamSchema = Joi.object({
  ...
  totalWeek: Joi.number().greater(0).optional(),
}).custom((value, helpers) => {
  if (value.date && value.day) {
    const providedDay = new Date(value.date).toLocaleDateString("en-US", {
      weekday: "long",
...
const editTeam = async (req, res, next) => {
  try {
    ...
    if (
      (payload.teamCode && !payload.day) ||
      (payload.day && !payload.teamCode)
    ) {
      return res
        .status(400)
        .json({ message: "Both team code and day are required to update" });
    }
    ...
    if (payload.teamCode && payload.day) {
      ...
      if (existingTeam) {
        return res
          .status(400)
          .json({ message: "Team with the same code and day already exists" });
      }
    }

    const transactions = await Transaction.find({ teamId: id });
    const collected = transactions.reduce(
      (sum, t) => sum + t.collectedAmount,
      0
    );
    const maxWeek =
      transactions.length > 0
        ? Math.max(...transactions.map((t) => t.week))
        : 0;
    ...
    res
      .status(200)
      .json({ message: "Team updated successfully", team, balanceWeek });
  } catch (error) {
    next(error);
  }
};
...
const createMemberSchema = Joi.object({
  ...
  // photo comes via multipart as req.file; body.photo string is ignored
  ...
  try {
    ...
    if (req.file && req.file.buffer) {
      const uploaded = await uploadToImageKit(
        ...
        {
          tags: ["member", String(team.teamCode)],
        }
      );
      payload.photo = uploaded.url;
      payload.photoId = uploaded.fileId;
    }
...
const editMemberSchema = Joi.object({
  ...
  // photo comes via multipart; ignore body.photo string
  ...
  try {
    ...
    if (req.file && req.file.buffer) {
      const uploaded = await uploadToImageKit(
        ...
        req.file.originalname,
        {
          tags: ["member", String(member.teamId)],
        }
      );
      const oldPhotoId = member.photoId;
      member.photo = uploaded.url;
      member.photoId = uploaded.fileId;
      if (oldPhotoId) {
        await deleteFromImageKit(oldPhotoId);
      }
    }
...
const deleteMember = async (req, res, next) => {
  try {
    ...
    if (member.photoId) {
      await deleteFromImageKit(member.photoId);
    }
...
const addTransaction = async (req, res, next) => {
  try {
    ...
    if (totalCollected > team.totalAmount) {
      return res.status(400).json({
        message: "Collected amount exceeds total amount for the team",
      });
    }
...
const deleteTransaction = async (req, res, next) => {
  try {
    ...
    team.collectedAmount = Math.max(
      0,
      team.collectedAmount - transaction.collectedAmount
    );
